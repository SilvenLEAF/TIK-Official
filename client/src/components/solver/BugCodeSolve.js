...
function BugCodeSolve(props) {
  ...
  console.log("BugCodeSolve -> props", props)
  const bugCodeId = props.match.params.bugCodeId;  
    
  const updateYourBugCode = async (e) =>{
    e.preventDefault();

    const updatedBugPost = { bugCodeId, solved: true }
  ...
  return (
    <div id="addBugCodesPage" className="container">
      ...
      <form onSubmit={ updateYourBugCode }>
        ...
        <button type="submit" className="btn blue" style={{marginRight: "20px"}} >Solved this <i className="fa fa-bug"></i></button>
